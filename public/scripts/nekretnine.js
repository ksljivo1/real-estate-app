    ...
    if (filtriraneNekretnine.length === 0) {
        ...
        filtriraneNekretnine.forEach(nekretnina => {
            ...
            detaljiElement.innerHTML = `
                ...
                <p>Lokacija: <a id=lokacijaAnchor${nekretnina.id} href="#">${lokacija}</a></p>
            `;
            ...
            const lokacijaAnchor = document.getElementById(`lokacijaAnchor${nekretnina.id}`)
            lokacijaAnchor.addEventListener('click', (l) => {
                PoziviAjax.getTop5Nekretnina(lokacija, (err, data) => {
                    ...
                    MarketingAjax.novoFiltriranje(
                        top5Nekretnina.map(nekretnina => nekretnina.id)
                    );
                    const naslov = document.getElementById('top5Naslov')
                    naslov.style.display = 'block'
                    naslov.innerText += (" " + lokacija)
                    filtrirajNekretnine(top5Nekretnina)
                })
            })
        });
    }
